...
@tweets_route.route("/")
def home():
    users_tab = parse_records(User.query.all())
    screen_names = []
    for i in range(0,len(users_tab)): 
        screen_names.append(users_tab[i]['screen_name'])
    return render_template("prediction_form.html", screen_names = users_tab)
...
@tweets_route.route("/see_users")
def list_users():
    
    users_tab = parse_records(User.query.all())
    screen_names = []
    # output_users = parse_records(users)
    for i in range(0,len(users_tab)): 
        screen_names.append(users_tab[i]['screen_name'])

    return render_template("user_list.html", screen_names = users_tab)
